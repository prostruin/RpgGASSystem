...
void AEnemyBase::InitClassDefaults()
{
	...
	{
		UE_LOG(LogTemp,Warning,TEXT("no character tag select this character %s"), *GetNameSafe(this));
	}
	else if (UCharacterClassInfo* ClassInfo = URPGAbilitySystemLibrary::GetCharacterClassDefaultInfo(this))
	{
		if (const FCharacterClassDefaultInfo* SelectedClass = ClassInfo->ClassDefaultsInfoMap.Find(CharacterTag))
		{
