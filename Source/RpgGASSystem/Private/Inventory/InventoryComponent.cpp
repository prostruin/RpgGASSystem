...
void UInventoryComponent::ReconstructInventoryMap(const FPackagedInventory& Inventory)
{
	...
	for (int32 i = 0; i < Inventory.ItemTags.Num(); ++i)
	{
		...
		GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Blue,
			FString::Printf(TEXT("Tag Added: %s // Qun Added : %d "), *Inventory.ItemTags[i].ToString(), Inventory.ItemQuantities[i]));
	}
}
...
{
	checkf(InventoryDefinitions, TEXT("No Inventory Definitions Inside Components %s"),*GetNameSafe(this));
